using Arinc.Spec424.Attributes;

namespace Arinc.Spec424.Records;

/// <summary>
/// <c>Airport SID/STAR/Approach</c> primary record.
/// </summary>
/// <remarks>See paragraph 4.1.9.1.</remarks>
[Continuation(39)]
public abstract record Procedure : Record424
{
    /// <summary>
    /// <c>Airport Identifier (ARPT)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.6.</remarks>
    [Field(7, 10)]
    [Link<AirportApproach, Airport>]
    [Link<StandardTerminalArrival, Airport>]
    [Link<StandardInstrumentDeparture, Airport>]
    public required string AirportIdentifier { get; init; }

    /// <summary>
    /// <c>ICAO Code (ICAO CODE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.14.</remarks>
    [Field(11, 12)]
    public required string IcaoCode { get; init; }

    /// <summary>
    /// <c>SID/STAR Route Identifier (SID/STAR IDENT)</c> or 
    /// <c>Approach Route Identifier (APPROACH IDENT)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.9 and 5.10.</remarks>
    [Field(14, 19)]
    public required string Identifier { get; init; }

    /// <summary>
    /// <c>Route Type (RT TYPE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.7.</remarks>
    [Character(20), Validation("\\w")]
    public required char RouteType { get; init; }

    /// <summary>
    /// <c>Transition Identifier (TRANS IDENT)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.11.</remarks>
    [Field(21, 25)]
    public required string TransitionIdentifier { get; init; }

    /// <summary>
    /// <c>Procedure Design Aircraft Category or Type</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.301.</remarks>
    [Character(26), Validation("\\w")]
    public required char AircraftCategory { get; init; }

    /// <summary>
    /// <c>Sequence Number (SEQ NR)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.12.</remarks>
    [Field(27, 29)]
    public required string SequenceNumber { get; init; }

    /// <summary>
    /// <c>Fix Identifier (FIX IDENT)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.13.</remarks>
    [Field(30, 34)]
    public required string FixIdentifier { get; init; }

    /// <summary>
    /// <c>ICAO Code (ICAO CODE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.14.</remarks>
    [Field(35, 36)]
    public required string FixIcaoCode { get; init; }

    /// <summary>
    /// <c>Section Code (SEC CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.4.</remarks>
    [Character(37), Validation("\\d|[A-Z]")]
    public required char FixSectionCode { get; init; }

    /// <summary>
    /// <c>Subsection Code (SUB CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.5.</remarks>
    [Character(38), Validation("\\d|[A-Z]")]
    public required char FixSubsectionCode { get; init; }

    /// <summary>
    /// <c>Waypoint Description Code (DESC CODE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.17.</remarks>
    [Field(40, 43)]
    public required string WaypointDescriptionCode { get; init; }

    /// <summary>
    /// <c>Turn Direction (TURN DIR)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.20.</remarks>
    [Character(44), Validation("[LR]")]
    public required char TumDirection { get; init; }

    /// <summary>
    /// <c>Required Navigation Performance</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.211.</remarks>
    [Field(45, 47)]
    public required string RNP { get; init; }

    /// <summary>
    /// <c>Path and Termination (PATH TERM)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.21.</remarks>
    [Field(48, 49)]
    public required string PathAndTermination { get; init; }

    /// <summary>
    /// <c>Turn Direction Valid (TDV)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.22.</remarks>
    [Character(50), Validation("[YN]")] // не уверен
    public required char TurnDirectionValid { get; init; }

    /// <summary>
    /// <c>Recommended NAVAID (RECD NAV)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.23.</remarks>
    [Field(51, 54)]
    public required string RecommendedNavaid { get; init; }

    /// <summary>
    /// <c>ICAO Code (ICAO CODE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.14.</remarks>
    [Field(55, 56)]
    public required string NavaidIcaoCode { get; init; }

    /// <summary>
    /// <c>ARC Radius (ARC RAD)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.204.</remarks>
    [Field(57, 62)]
    public required string? ArcRadius { get; init; }

    /// <summary>
    /// <c>Theta (THETA)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.24.</remarks>
    [Field(63, 66)]
    public required string? Theta { get; init; }

    /// <summary>
    /// <c>Rho (RHO)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.25.</remarks>
    [Field(67, 70)]
    public required string? Rho { get; init; }

    /// <summary>
    /// <c>Outbound Magnetic Course (OB MAG CRS)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.26.</remarks>
    [Field(71, 74)]
    public required string MagneticCourse { get; init; }

    /// <summary>
    /// <c>Route Distance From, Holding Distance/Time (RTE DIST FROM, HOLD DIST/TIME)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.27.</remarks>
    [Field(75, 78)]
    public required string? RouteDistanceOrHoldingTime { get; init; }

    /// <summary>
    /// <c>Section Code (SEC CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.4.</remarks>
    [Character(79), Validation("\\d|[A-Z]")]
    public required char? RecdNavSection { get; init; }

    /// <summary>
    /// <c>Subsection code (SUB CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.5.</remarks>
    [Character(80), Validation("\\d|[A-Z]")]
    public required char? RecdNavSubsection { get; init; }

    /// <summary>
    /// <c>Holding Pattern/Race Track Course Reversal Leg Inbound/Outbound Indicator</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.298.</remarks>
    [Character(81), Validation("O|I")]
    public required char LegDirectionIndicator { get; init; }

    /// <summary>
    /// <c>Altitude Description (ALT DESC)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.29.</remarks>
    [Character(83), Validation("\\+|-|@|B|C|D|G|O|V|X|Y")]
    public required char AltitudeDescription { get; init; }

    /// <summary>
    /// <c>ATC Indicator (ATC)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.81.</remarks>
    [Character(84), Validation("B|H|L| ")]
    public required char IndicatorATC { get; init; }

    /// <summary>
    /// <c>Altitude/Minimum Altitude</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.30.</remarks>
    [Field(85, 89)]
    public required string Altitude1 { get; init; }

    /// <summary>
    /// <c>Altitude/Minimum Altitude</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.30.</remarks>
    [Field(90, 94)]
    public required string Altitude2 { get; init; }

    /// <summary>
    /// <c>Transition Altitude/Level (TRANS ALTITUDE/LEVEL)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.53.</remarks>
    [Field(95, 99)]
    public required string TransitionAltitude { get; init; }

    /// <summary>
    /// <c>Speed Limit (SPEED LIMIT)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.72.</remarks>
    [Field(100, 102)]
    public required string SpeedLimit { get; init; }

    /// <summary>
    /// <c>Vertical Angle (VERT ANGLE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.70.</remarks>
    [Field(103, 106)]
    public required string VerticalAngel { get; init; }

    /// <summary>
    /// <c>Center Fix (CENTER FIX)
    /// Procedure Turn (PROC TURN)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.144 or 5.271.</remarks>
    [Field(107, 111)]
    public required string FixProcedureIndicator { get; init; }

    /// <summary>
    /// <c>Multiple Code (MULTI CD)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.130 or 5.271 .</remarks>
    [Character(112)]
    public required char? CodeTaaIndicator { get; init; }

    /// <summary>
    /// <c>ICAO Code (ICAO CODE)</c> field.
    /// </summary>
    /// <remarks>See paragraph 5.14.</remarks>
    [Field(113, 114)]
    public required string? FixProcedureIcaoCode { get; init; }

    /// <summary>
    /// <c>Section Code (SEC CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.4.</remarks>
    [Character(115), Validation("\\d|[A-Z]")]
    public required char? FixProcedureSectionCode { get; init; }

    /// <summary>
    /// <c>Subsection Code (SUB CODE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.5.</remarks>
    [Character(116), Validation("\\d|[A-Z]")]
    public required char? FixProcedureSubsectionCode { get; init; }

    /// <summary>
    /// <c>GNSS/FMS Indicator (GNSS/FMS IND)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.222.</remarks>
    [Character(117), Validation("1|2|3|4|5|A|B|C|D|P|G|L")]
    public required char GnssFmsIndication { get; init; }

    /// <summary>
    /// <c>Speed Limit Description (SLD)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.261.</remarks>
    [Character(118), Validation("@|\\+|-")]
    public required char SpeedLimitDescription { get; init; }

    /// <summary>
    /// <c>Route Type (RT TYPE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.7.</remarks>
    [Character(119), Validation("A|C|D|H|P|R|S|T")]
    public required char RouteQualifier1 { get; init; }

    /// <summary>
    /// <c>Route Type (RT TYPE)</c> character.
    /// </summary>
    /// <remarks>See paragraph 5.7.</remarks>
    [Character(120), Validation("A|C|D|H|P|R|S|T")]
    public required char RouteQualifier2 { get; init; }
}
